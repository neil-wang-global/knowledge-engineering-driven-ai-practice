...
| 5. Skill / Knowledge / Rules | 10 分钟 | 比较 Skill、知识工程和 Rules 三种知识注入方式。 |
...
### 5. Skill / Knowledge / Rules：三种知识注入方式

Skill、知识工程和 Rules 都是在解决同一个问题：怎样把经验、判断标准和流程约束，变成模型可读取、可执行、可检查的运行时知识。

但它们不在同一层。

第一种是 Skill。Skill 是能力层。它把一类任务的触发条件、输入、读取内容、执行步骤、输出位置、停止条件、权限边界和验证方式封装起来。它的优点是灵活，缺点是调度带有随机性。Agent 需要判断当前任务是否匹配某个 Skill，是否要加载，什么时候加载。只要判断发生在模型侧，就会有漏触发、误触发和触发时机不稳定的问题。

第二种是 Knowledge Engineering。知识工程是资产层。它不只是某个流程说明，而是把项目知识体系化：Story、ADR、Spec、API Contract、Plan、State、Review、Eval、Capability、Reference 都有自己的位置、字段和关系。它适合沉淀长期知识，适合跨角色复用，也适合让 Agent 沿着对象关系读取上下文。知识工程解决的是“项目知识怎样被组织成系统”。

第三种是 Rules。Rules 是控制层。这里说的 Rules 不是普通建议，而是 Claude 或 harness 层必定执行的规则，接近 hooks、permission gate、stop condition、required check。它不依赖模型想起来，也不依赖 Agent 判断要不要加载。比如提交前必须跑 `git diff --check`，高风险命令必须请求确认，进入某个阶段必须读取指定文件，输出完成声明前必须先验证。这类动作只要是“必须发生”，就不应该只写进 Skill，而应该下沉为 Rule、Hook 或 Harness gate。

所以三者可以这样比较：Skill 调度随机，适合封装可复用能力；知识工程调度更适合项目知识的体系化，适合组织长期上下文；Rules 调度确定，适合 harness，适合承载安全边界、流程硬约束和强制校验。

一句话总结：Knowledge 是资产层，Skill 是能力层，Rules 是控制层。AI-native 系统不能只靠其中一种。知识工程让系统有上下文，Skill 让系统有方法，Rules 让系统有不可绕过的底线。

一个好的工程文档可以这样验收：触发清楚，输入明确，步骤可执行，输出可消费，验证内置，权限清楚，能减少重复解释，能防止同类错误复发。如果某条要求必须每次执行，就不要只写在文档里，要变成 Rule。
...
13. 强状态驱动高度自治
14. Skill / Knowledge / Rules：三种知识注入方式
15. Knowledge 是资产层，Skill 是能力层，Rules 是控制层
