...
我自己的判断是，强模型确实会把一部分 planning、tool use、agentic orchestration 吃进模型内部。过去需要外部提示词和流程补足的能力，现在一部分已经变成模型能力。但这不代表工程方法失效。相反，工程方法的价值正在从“能力补丁”迁移到“治理工具”。这与归档文章《当顶尖 Agent 模型越来越会自己干活，Prompt Engineering、SDD、MAS 还剩什么价值》中的判断一致：方法论的必要性开始分层，取决于模型能力、任务开放度、预算、容错要求和治理需求。

今天很多团队的现实问题，已经不只是模型不会写代码，而是模型太快开始写代码。需求还没澄清，边界还没想清，验证还没设计好，模型已经开始实现。尤其在一些中低端模型、上下文不足或反馈不足的场景里，模型容易过早收束：它会给出一个流畅的方案，也会生成看起来合理的代码，但没有充分探索异常路径、权限边界、状态一致性和验收条件。这里我把它作为工程经验来讲，不把它说成某个统计研究结论。

SDD 的价值就在这里。它不是神秘地提高模型智力，而是把“先想清楚”制度化。它要求我们把意图、约束、中间产物、检查点和验收标准显化。强模型可以更聪明地执行，但系统仍然需要知道：什么时候可以开始做，什么时候必须停下来问，什么时候必须测试，什么时候必须回写文档，什么时候必须交给人验收。

所以，AI 编程不是推翻软件工程，而是放大软件工程基本功。AI 越快，流程越重要；AI 越能做，状态越重要；AI 越像一个执行者，我们越需要把它放在清晰的任务边界、权限边界和验收边界里。
...
## 第 3 章：ProMaster / POMASA 与 MAS 的强状态

接下来讲 ProMaster / POMASA 与 MAS。这里需要先做一个降级说明：当前可追溯材料主要来自 POMASA，ProMaster 的独立证据不足，所以我会保留章节名，但具体事实主要围绕 POMASA 以及这一类强状态多智能体实践来讲。
