...
2. SoC、抽象、目录结构分别对应上下文隔离、上下文压缩、上下文索引。
3. MindFlow 说明软件结构可以迁移到认知结构：Soul、Learning、Mind、Capability、Plan/Step、Task State。
4. 知识工程说明知识不是文档仓库，而是 user journey 里的 story diff。
5. POMASA 作为声明式 MAS 样本，说明 Agent 系统需要 Blueprint、方法论、文件总线、强状态和可追溯数据血缘。
6. Skill 与工程文档是可执行知识，不是说明书。
7. 敏捷与单仓库协作：BA/DEV/QA/UI/Agent 是角色，不是固定人；敏捷工件变成可执行、可检查、可回流的强状态。
...
- MindFlow：第 4 章，引用 [SRC-001][SRC-002][SRC-003][SRC-004]。
- POMASA：第 6 章，引用 [SRC-005][SRC-006][SRC-007][SRC-008]。
- 上下文管理：第 3 章，引用 [SRC-024][SRC-002]。
- Harness Engineering：第 10、12、13 章，引用 [SRC-020][SRC-021][SRC-050][SRC-053][SRC-054]。
- Eval：第 11 章，引用 [SRC-026][SRC-052]。
- 敏捷/单仓库：第 8 章，引用 [USER-INPUT][SRC-025]。
